...
<template>
  <Toolbar class="p-3 rounded-5 my-2 mx-4">
    <template #start>
      <div class="flex items-center gap-2">
        ...
        <RouterLink to="/admin/doctorsDashboard">
          <Button label="Admin" text plain />
        </RouterLink>
        <RouterLink to="/register">
          <Button label="Register" text plain />
        </RouterLink>
        <RouterLink to="/chat">
    ...
    <template #end>
      <div class="d-flex justify-content-end align-items-center gap-2" data-bs-toggle="dropdown" aria-expanded="false">
        <Button text plain>
          <p class="m-0" v-if="authStore.loggedInUser.firstName">
            <span>{{ authStore.isDoctorOn() ? 'Dr. ' : '' }}</span>
            {{ authStore.loggedInUser.firstName + ' ' + authStore.loggedInUser.lastName }}
          </p>
          <img v-if="authStore.isDoctorOn()"
               :src="authStore.loggedInDoctor.imageFilePath"
               alt="dr image" class="logged-in-dr-image" />
        </Button>
      </div>
      <ul class="dropdown-menu">
        <li>
          <RouterLink to="/doctorProfile" class="dropdown-item">Profile</RouterLink>
        </li>
